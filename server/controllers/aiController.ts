 ...
 */
export function normalizeCookingQuery(rawPrompt: string): {
  clean: string;
  normalized: string;
  detectedDish: string;
  hasCookingIntent: boolean;
  isOffensive: boolean;
} {
  ...
  const hasCookingIntent =
    foodKeywords.some((w) => new RegExp(`\\b${escapeRegex(w)}\\b`, "i").test(normalized)) ||
...
const RECIPE_KNOWLEDGE_BASE: Array<{
  id: string;
  name: string;
  aliases: string[];
  description: string;
 ...
 */
function isGreetingOrNonFood(prompt: string): { isNonFood: boolean; reason: string; isOffensive: boolean } {
 ...
 */
async function parseRecipeWithGemini(prompt: string, servings: number = 4) {
 ...
 */
function parseRecipeFallback(rawPrompt: string, requestedServings: number = 4) {
  ...
  const extractedIngredients: any[] = [];
  const catalogVocab = [
 ...
 */
export const recipeToCart = async (req: Request, res: Response) => {
  try {
    ...
    if (!recipeData || recipeData.isValidRecipe === false) {
      return res.status(200).json({
        ...
        isGreeting: true,
        message:
          recipeData?.clarificationMessage ||
    ...
    const productsWithDiscount = dbProducts.map((p: any) => {
      const discount =
    ...
    for (const ingredient of recipeData.ingredients) {
      const searchTerms: string[] = Array.isArray(ingredient.searchTerms)
    ...
    const totalBundlePrice = matchedItems.reduce(
      (sum, item) => sum + item.product.price * item.quantity,
      0
    );
    const totalOriginalPrice = matchedItems.reduce(
