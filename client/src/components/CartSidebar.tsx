          ...
          {/* Sidebar */}
            ...
            <div className="flex items-center justify-between p-4 border-b border-app-border">
              <div className="flex items-center gap-2">
                <ShoppingBagIcon className="size-5 text-app-green dark:text-zinc-100" />

                <h2 className="text-lg font-medium text-app-green dark:text-zinc-100">
                  Your Cart
                </h2>

                <span className="px-2 py-0.5 text-xs font-semibold bg-app-cream dark:bg-zinc-800 text-app-green dark:text-zinc-300 rounded-full">
                  {items.length} items
                </span>
              </div>
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-4">
              {items.length === 0 ? (
                ...
                items.map((item) => (
                  <div
                    key={item.product.id}
                    className="flex bg-app-cream/60 dark:bg-zinc-800/60 rounded-xl gap-4 p-2 items-center"
                  >
                    <img
                    ...
                    <div className="flex-1 min-w-0">
                      <h4 className="text-sm font-semibold truncate text-app-green dark:text-zinc-100">
                        {item.product.name}
                      </h4>
                      ...
                      <div className="flex items-center justify-between mt-2">
                        <div className="flex items-center gap-1.5">
                          <button
                            ...
                            className="size-7 rounded-lg bg-white dark:bg-zinc-800 border border-app-border dark:border-zinc-700 flex-center hover:bg-zinc-100 dark:hover:bg-zinc-700 transition-colors cursor-pointer"
                          >
                            <MinusIcon className="size-3 dark:text-zinc-100" />
                          </button>
                          <span className="text-sm font-semibold w-6 text-center dark:text-zinc-100">
                            {item.quantity}
                          </span>
                          ...
                          <button
                            ...
                            className="size-7 rounded-lg bg-white dark:bg-zinc-800 border border-app-border dark:border-zinc-700 flex-center hover:bg-zinc-100 dark:hover:bg-zinc-700 transition-colors cursor-pointer"
                          >
                            <PlusIcon className="size-3 dark:text-zinc-100" />
                          </button>
                        </div>
            ...
            {/* Footer */}
            {items.length > 0 && (
              <div className="p-5 border-t border-app-border dark:border-zinc-800 space-y-4 bg-white dark:bg-zinc-900">
                ...
                <div className="flex justify-between text-base font-semibold border-t border-app-border dark:border-zinc-800 pt-3">
                  <span className="text-app-green dark:text-zinc-100">Grand Total:</span>
                  <span className="text-app-green dark:text-zinc-100">
                    {currency}
